...
async function submeter() {
  ...
  try {
    ...
    erro.value = e instanceof Error ? e.message : 'Erro ao iniciar sessão'
  } finally {
    loading.value = false
  }
}
    ...
    <div class="w-full max-w-sm bg-white border border-slate-200 rounded-xl shadow-sm p-8">
      ...
      <form @submit.prevent="submeter" class="space-y-4">
        ...
        <button type="submit" :disabled="loading"
                class="w-full bg-[#2563eb] hover:bg-[#1d4ed8] disabled:opacity-60 text-white text-sm font-semibold py-2 rounded-md transition-colors">
